...
try {
  ...
  await check('tools/list: 23 tools; fx_snapshot/fx_form gained deep param', { result: { content: [{ text: '' }] } }, (t, e) => {
    if (e) return false;
    const names = list.result.tools.map((t) => t.name);
    return names.length === 23 && schema('fx_snapshot').deep && schema('fx_form').deep && schema('fx_click').ref;
  });
